...
import { join, resolve, relative, sep, basename, isAbsolute, win32 } from "node:path";
...
export class ToolExecutor {
  ...
  _real(virtual) {
    ...
    const normalizedVirtual = virtual.replace(/\\/g, "/");
    let candidate;
    if (normalizedVirtual === "/codebase" || normalizedVirtual.startsWith("/codebase/")) {
      const rel = normalizedVirtual.slice("/codebase".length).replace(/^\/+/, "");
      candidate = resolve(this.root, rel);
    } else {
      if (isAbsolute(virtual) || win32.isAbsolute(virtual) || normalizedVirtual.startsWith("//")) {
        throw new Error(`path is outside project root: ${virtual}`);
      }
      candidate = resolve(this.root, virtual);
    }

    this._assertWithinRoot(candidate, virtual);
    if (existsSync(candidate)) {
      const realCandidate = realpathSync(candidate);
      this._assertWithinRoot(realCandidate, virtual);
      return realCandidate;
    }
    return candidate;
  }

  _assertWithinRoot(candidate, original) {
  ...
  async execCommandAsync(cmd) {
    ...
    try {
      ...
      switch (t) {
        case "rg":
          return await this.rgAsync(cmd.pattern, cmd.path, cmd.include || null, cmd.exclude || null);
        case "readfile":
