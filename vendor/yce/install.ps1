...
# Contract: .env lives at YCE skill root (this install.ps1 directory), NEVER under vendor/yce-engine/.
$EnvFile = Join-Path $ScriptDir ".env"
$WrongEngineEnvFile = Join-Path $ScriptDir "vendor\yce-engine\.env"
...
function Move-EnvFromEngineIfNeeded {
  param(
    [string]$SkillEnv = $EnvFile,
    [string]$EngineEnv = $WrongEngineEnvFile
  )
  if (-not (Test-Path $EngineEnv)) { return }
  if (-not (Test-Path $SkillEnv)) {
    $parent = Split-Path -Parent $SkillEnv
    if (-not (Test-Path $parent)) { New-Item -ItemType Directory -Force -Path $parent | Out-Null }
    Copy-Item $EngineEnv $SkillEnv -Force
    Write-Ok ".env 已从错误位置迁移到 skill 根目录: $SkillEnv"
  } else {
    Merge-EnvMissingKeys -SourceEnv $EngineEnv -TargetEnv $SkillEnv
    Write-Warn "发现 yce-engine 目录下的 .env；密钥应以 skill 根目录为准: $SkillEnv"
  }
  Remove-Item $EngineEnv -Force -ErrorAction SilentlyContinue
  Write-Ok "已删除错误位置: $EngineEnv"
}
...
function Write-RuntimeConfig {
  ...
  Move-EnvFromEngineIfNeeded
  Write-Host "Generating .env at YCE skill root (not yce-engine)..."
  ...
  if (Test-Path $WrongEngineEnvFile) {
    Remove-Item $WrongEngineEnvFile -Force -ErrorAction SilentlyContinue
    Write-Warn "已清理错误位置: $WrongEngineEnvFile"
  }
