...
test("auto 不会因 query 含最新/官方文档等字样自动联网", async () => {
  const originalFetch = global.fetch;
  let fetchCalled = 0;
  global.fetch = async () => {
    fetchCalled += 1;
  ...
  try {
    ...
    // Without code-search keywords this falls through to enhance path → no token → search fallback.
    // Critical: no network unless AI passes --mode network or --with-network.
...
test("AI 显式 --with-network 时在代码检索上叠加联网", async () => {
  const originalFetch = global.fetch;
  let fetchCalled = 0;
  global.fetch = async () => {
    fetchCalled += 1;
...
test("mode=network 只做联网不作代码检索", async () => {
  const originalFetch = global.fetch;
  let fetchCalled = 0;
  global.fetch = async () => {
    fetchCalled += 1;
    return {
      ...
      async json() {
        return {
          status: "succeeded",
          evidence: [{ title: "最新事实", url: "https://example.com/latest" }],
          summaries: [{ text: "调研摘要" }],
          providerRuns: [{ provider: "fixture", status: "succeeded" }],
          failures: [],
          usage: {},
  ...
  try {
    const result = await orchestrate({
      mode: "network",
      query: "What is the latest official guidance for this library",
      cwd: repoRoot,
      history: "",
      noSearch: false,
      rawEvents: false,
