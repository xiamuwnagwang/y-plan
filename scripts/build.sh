...
read_skill_version() {
  ...
  [[ -f "$skill" ]] || return 1
  grep -m1 '^version:' "$skill" 2>/dev/null | sed 's/version:[[:space:]]*//' | tr -d '[:space:]'
}

# Version source order: CLI arg > SKILL.md > git tag > dev
if [[ -n "${1:-}" ]]; then
  VERSION="$1"
else
  VERSION="$(read_skill_version || true)"
  if [[ -z "$VERSION" ]]; then
    VERSION="$(git -C "$PROJECT_ROOT" describe --tags --abbrev=0 2>/dev/null || echo "dev")"
  else
    # normalize to vX.Y.Z for archive names when SKILL.md has bare semver
